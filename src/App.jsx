...
function HomeRP(p){
  ...
  var miniBtn=function(ic,lb,fn,cl){return(<button onClick={fn} style={{background:cl+'18',border:'0.5px solid '+cl+'44',borderRadius:8,padding:'7px 4px',cursor:'pointer',fontSize:16,fontWeight:600,color:cl,textAlign:'center'}}>{ic}<div style={{fontSize:15,marginTop:2}}>{lb}</div></button>);};
      ...
      {(function(){var noRealGP=NO_GP_USERS.includes(user.id)||(!!group&&!!group.isVirtual);if(noRealGP)return null;return group&&(
        <div style={{marginBottom:4}}>
          ...
          <Card style={{padding:0,overflow:'hidden'}}>
            <div style={{background:'#fff',borderBottom:'1px solid '+BO,padding:'12px 14px 10px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                <div>
                  <p style={{margin:0,fontWeight:700,fontSize:18,color:'#071F33'}}>{group.nombre}</p>
                  <p style={{margin:'3px 0 0',fontSize:16,color:MU}}>{group.sector||user.sector} · {mbPP.length} {user.genero==='F'?'hermanas':'hermanos'}</p>
                </div>
                <span style={{background:(pct>=80?G:pct>=50?W:R)+'22',color:pct>=80?G:pct>=50?W:R,fontSize:16,padding:'3px 8px',borderRadius:20,fontWeight:700,whiteSpace:'nowrap'}}>{pct}%</span>
              </div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',borderBottom:'1px solid '+BO}}>
              <div style={{padding:'10px 8px',textAlign:'center',borderRight:'1px solid '+BO,background:BG,borderRadius:10,margin:'0 4px 0 8px'}}>
                <div style={{fontSize:27,fontWeight:700,color:P}}>{mbPP.length}</div>
                <div style={{fontSize:15,color:MU}}>hermanos</div>
              </div>
              <div style={{padding:'10px 8px',textAlign:'center',borderRight:'1px solid '+BO,background:BG,borderRadius:10,margin:'0 4px'}}>
                <div style={{fontSize:27,fontWeight:700,color:G}}>{semR.length}</div>
                <div style={{fontSize:15,color:MU}}>Pastoreos realizados</div>
              </div>
              <div style={{padding:'10px 8px',textAlign:'center',background:BG,borderRadius:10,margin:'0 8px 0 4px'}}>
                <div style={{fontSize:27,fontWeight:700,color:'#071F33'}}>{meta}</div>
                <div style={{fontSize:15,color:MU}}>meta</div>
              </div>
            </div>
            <div style={{padding:'10px 14px 4px'}}>
              <div style={{background:BO,borderRadius:20,height:6,overflow:'hidden'}}>
                <div style={{width:Math.min(100,pct)+'%',height:'100%',background:pct>=80?G:pct>=50?W:R,borderRadius:20}}></div>
              </div>
              <p style={{margin:'5px 0 0',fontSize:15,color:MU,textAlign:'right'}}>{semR.length} de {meta} pastoreos{meta-semR.length>0?' · faltan '+(meta-semR.length):' · ¡meta cumplida! 🎉'}</p>
            </div>
            <div style={{padding:'10px 14px 12px'}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:6,marginBottom:6}}>
                {miniBtn('👥','Mi Grupo',function(){go('grupo');},A)}
                {miniBtn('📅','Agendar',function(){go('agendar');},'#718096')}
                {miniBtn('✅','Registrar',function(){go('registrar');},G)}
              </div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
                {miniBtn('📊','Progreso',function(){go('progreso');},'#1A6FA8')}
                {miniBtn('👁','Supervisión',function(){go('sup');},'#1A6FA8')}
              </div>
            </div>
          </Card>
        </div>
        ...
        <div style={{marginBottom:4}}>
          ...
          <Card style={{padding:0,overflow:'hidden'}}>
            <div style={{background:'#fff',borderBottom:'1px solid '+BO,padding:'12px 14px 10px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                <div>
                  <p style={{margin:0,fontWeight:700,fontSize:18,color:'#071F33'}}>{myGroup.nombre}</p>
                  <p style={{margin:'3px 0 0',fontSize:16,color:MU}}>{myGroup.sector} · {myGroupMb} {user.genero==='F'?'hermanas':'hermanos'}</p>
                </div>
                <span style={{background:(myGroupPct>=80?G:myGroupPct>=50?W:R)+'22',color:myGroupPct>=80?G:myGroupPct>=50?W:R,fontSize:16,padding:'3px 8px',borderRadius:20,fontWeight:700,whiteSpace:'nowrap'}}>{myGroupPct}%</span>
              </div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',borderBottom:'1px solid '+BO}}>
              <div style={{padding:'10px 8px',textAlign:'center',borderRight:'1px solid '+BO,background:BG,borderRadius:10,margin:'0 4px 0 8px'}}>
                <div style={{fontSize:27,fontWeight:700,color:P}}>{myGroupMb}</div>
                <div style={{fontSize:15,color:MU}}>{user.genero==='F'?'hermanas':'hermanos'}</div>
              </div>
              <div style={{padding:'10px 8px',textAlign:'center',borderRight:'1px solid '+BO,background:BG,borderRadius:10,margin:'0 4px'}}>
                <div style={{fontSize:27,fontWeight:700,color:G}}>{myGroupReal}</div>
                <div style={{fontSize:15,color:MU}}>Pastoreos realizados</div>
              </div>
              <div style={{padding:'10px 8px',textAlign:'center',background:BG,borderRadius:10,margin:'0 8px 0 4px'}}>
                <div style={{fontSize:27,fontWeight:700,color:'#071F33'}}>{myGroupMeta}</div>
                <div style={{fontSize:15,color:MU}}>meta</div>
              </div>
            </div>
            <div style={{padding:'10px 14px 4px'}}>
              ...
              <p style={{margin:'5px 0 0',fontSize:15,color:MU,textAlign:'right'}}>{myGroupReal} de {myGroupMeta} pastoreos{myGroupMeta-myGroupReal>0?' · faltan '+(myGroupMeta-myGroupReal):' · ¡meta cumplida! 🎉'}</p>
            </div>
            <div style={{padding:'10px 14px 12px'}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:6,marginBottom:6}}>
                {miniBtn('👥','Mi Grupo',function(){go('grupo');},A)}
                {miniBtn('📅','Agendar',function(){go('agendar');},'#718096')}
                {miniBtn('✅','Registrar Cita',function(){go('registrar');},G)}
              </div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
                {miniBtn('📊','Progreso',function(){go('progreso');},'#1A6FA8')}
                {miniBtn('👁','Mis Citas',function(){go('misup');},'#1A6FA8')}
              </div>
            </div>
          </Card>
        </div>
...
function Cadena(p){
  ...
  return (
    <div style={{padding:'20px 16px 100px',fontFamily:'system-ui'}}>
      ...
      {upcoming.length>0&&(<div><p style={{fontSize:17,fontWeight:700,color:P,margin:'0 0 8px'}}>PRÓXIMAS ({upcoming.length})</p>{upcoming.map(function(m){var t=getTipo(m.tipo),marked=selId===m.id;return(<Card key={m.id} style={{marginBottom:10,border:marked?'2px solid '+P:undefined}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:marked?10:0}}><div><div style={{display:'flex',alignItems:'center',gap:6,marginBottom:3}}><span style={{fontSize:21}}>{t.icon}</span><span style={{background:t.color+'22',color:t.color,fontSize:16,padding:'2px 8px',borderRadius:20,fontWeight:700}}>{t.label}</span></div><p style={{margin:0,fontSize:18,fontWeight:600,color:'#071F33'}}>{partLabel(m)}</p><p style={{margin:'2px 0 0',fontSize:16,color:MU}}>{fmt(m.fecha)} · {m.hora}</p></div><Btn small variant={marked?'outline':'success'} onClick={function(){setSelId(marked?null:m.id);setRNotas('');setRPautas('');}}>{marked?'Cancelar':'✅ Realizada'}</Btn></div>{marked&&(<div><div style={{marginBottom:8}}><div style={{fontSize:17,color:MU,fontWeight:600,marginBottom:4}}>Nota / resumen</div><textarea value={rNotas} onChange={function(e){setRNotas(e.target.value);}} style={inp({minHeight:60,resize:'vertical'})} placeholder='¿Cómo fue la reunión?'/></div>{(m.tipo==='retro1'||m.tipo==='retro2')&&(<div style={{marginBottom:10}}><div style={{fontSize:17,color:MU,fontWeight:600,marginBottom:4}}>Pautas pastorales</div><textarea value={rPautas} onChange={function(e){setRPautas(e.target.value);}} style={inp({minHeight:80,resize:'vertical'})} placeholder='Pautas para fortalecer el PPG y el PPP...'/></div>)}<Btn variant='success' onClick={function(){confirmar(m);}} style={{width:'100%'}}>✅ Confirmar como Realizada</Btn></div>)}</Card>);})}</div>)}
...
function ViewGrupo(p){
  ...
  var [tab,setTab]=useState('hermanos');
  var TABS=[{id:'hermanos',lb:'👥 Hermanos'},{id:'ppg',lb:'📋 PPG'},{id:'ppp',lb:'📝 PPP'},{id:'asist',lb:'✅ Asistencia'},{id:'reuniones',lb:'📅 Reuniones'}];
  return (
    <div style={{fontFamily:'system-ui'}}>
